...
export default async function dbUpdate(settings, req) {
  ...
  const existingData = await loadAllValues(settings.redisKey);
  ...
  await redis.lset(`${appConfig.projectName}_${settings.redisKey}`, existingValueIndex, JSON.stringify(value));
  return true;
}
